...
import { ChevronRight, MapPin, Search, Tag, Trash2, X } from "lucide-react";
  ...
  return (
    <aside className="flex h-full w-full flex-col overflow-hidden border-r border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900 sm:w-80">
              ...
              <li key={`${hit.type}-${hit.name}-${hit.district ?? ""}`}>
                ...
                >
                  ...
                  <span className="shrink-0 text-xs text-slate-400">
                    {hit.type === "province" ? "Province" : hit.type === "district" ? "District" : hit.district}
                  </span>
                </button>
              </li>
            ))}
      ...
      {!loading && (selectedProvince || selectedDistrict) && (
        <div className="flex flex-wrap items-center gap-1 border-b border-slate-100 px-3 py-2 text-xs text-slate-500 dark:border-slate-800 dark:text-slate-400">
          <button onClick={() => onSelectProvince("")} className="hover:text-slate-800 dark:hover:text-slate-200">
            Nepal
          </button>
          {selectedProvince && (
            <>
              ...
              <button
                ...
                className={cn(
                  ...
                  !selectedDistrict && "font-medium text-slate-800 dark:text-slate-200"
                )}
              >
                {selectedProvince}
              </button>
          ...
          {selectedDistrict && (
            <>
              <ChevronRight className="h-3 w-3" />
              <span className="font-medium text-slate-800 dark:text-slate-200">{selectedDistrict}</span>
            </>
          )}
        </div>
      )}
